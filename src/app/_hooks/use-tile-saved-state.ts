...
export function useTileSaveState(tileId: string, authUserId: string) {
  ...
  const saveStateQuery = useQuery({
    queryKey: queryKeys.tileSaveState(tileId, authUserId),
    queryFn: () => fetchSaveTile(authUserId, tileId),
    initialData: () => queryClient.getQueryData(queryKeys.tileSaveState(tileId, authUserId)),
    staleTime: Infinity,
  ...
  const toggle = useMutation({
    ...
    onMutate: async ({ authUserId, isSaved }) => {
      ...
      await queryClient.cancelQueries({ queryKey: queryKeys.tileSaveState(tileId, authUserId) })

      const previousSaveState = queryClient.getQueryData(queryKeys.tileSaveState(tileId, authUserId))

      // Optimistic update
      queryClient.setQueryData(queryKeys.tileSaveState(tileId, authUserId), {
        authUserId,
        tileId,
        isSaved,
      })
    ...
    onError: (err, variables, context) => {
      if (context?.previousSaveState) {
        queryClient.setQueryData(queryKeys.tileSaveState(tileId, variables.authUserId), context.previousSaveState)
      }
    },
    // Always refetch after error or success to ensure we have the latest data
    onSettled: (data, error, variables) => {
      // Invalidate the save state for this specific tile/user combination
      queryClient.invalidateQueries({ queryKey: queryKeys.tileSaveState(tileId, variables.authUserId) })

      // Invalidate the authuser's saved tiles
      queryClient.invalidateQueries({ queryKey: queryKeys.userTiles(variables.authUserId) })
    },
  })
